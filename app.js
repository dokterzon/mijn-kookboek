...
const ANTH_URL = "/.netlify/functions/claude";
...
async function dbLoadPantry() {
  const { data, error } = await supa.from("pantry").select("*").eq("id", 1).single();
...
function App() {
  ...
  const fetchWithAI = async () => {
    ...
    const suggestionsSchema = '[{"title":"string","source":"sitenaam.nl","rating":"4.8","reviews":"1.203","time":"X min","difficulty":"Makkelijk|Gemiddeld|Moeilijk","emoji":"emoji","description":"één zin","color":"#hexkleur"}]';
    ...
    const headers = {
      ...
      "x-api-key": "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",
    ...
    try {
      ...
      const suggestPrompt = isUrl
        ? 'Zoek het recept op van deze URL: ' + aiUrl + '. Geef 1 resultaat terug als JSON array: ' + suggestionsSchema + ' Geen markdown, alleen JSON.'
        : 'Zoek 5 populaire goed beoordeelde recepten voor "' + aiUrl + '" van verschillende websites. Geef terug als JSON array: ' + suggestionsSchema + ' Geen markdown, alleen JSON.';
      ...
      if (!match) throw new Error("Geen suggesties ontvangen");
